...
void MyVisitor::notImplemented(const std::string& str) {
    stringTree.clear();
    stringTree.emplace_back(str + " is not implemented");
}
...
antlrcpp::Any MyVisitor::visitBlock(JavaScriptParser::BlockContext *ctx) {
    offset++;
    auto text = getTermMode(ctx->OpenBrace());
    addToTree("BlockStatement " + text);
...
antlrcpp::Any MyVisitor::visitEmptyStatement(JavaScriptParser::EmptyStatementContext *ctx){
    offset++;
    addToTree("EmptyStatement");
    auto vc = visitChildren(ctx);
    offset--;
    return vc;
}

antlrcpp::Any MyVisitor::visitExpressionStatement(JavaScriptParser::ExpressionStatementContext *ctx) {
    offset++;
    std::string text = "ExpressionStatement";
    addToTree(text);
    auto vc = visitChildren(ctx);
    offset--;
    return vc;
...
//antlrcpp::Any MyVisitor::visitExpressionSequence(JavaScriptParser::ExpressionSequenceContext *ctx) {
//    offset++;
//    addToTree("ExpressionSequence");
//    auto vc = visitChildren(ctx);
//    offset--;
//    return vc;
//}
...
antlrcpp::Any MyVisitor::visitBitShiftExpression(JavaScriptParser::BitShiftExpressionContext *ctx) {
    ...
    if (text.empty()) {
        text = getTermMode(ctx->RightShiftLogical());
    }
...
antlrcpp::Any MyVisitor::visitAssignmentOperator(JavaScriptParser::AssignmentOperatorContext *ctx) {
    offset++;
    std::string text = "AssignmentExpression " + getTermMode(ctx->PowerAssign());
    ...
    text += getTermMode(ctx->MinusAssign());
...
void MyVisitor::addWhiteSpace(std::string &str) const {
    for (int i = 0; i < offset * OFFSET_TABS; i++) {
